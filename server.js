...
const responses = {
  ...
  "おはよう": ["<b>おはようブラザー！いい朝だな！</b>"],
  "おやすみ": ["<b>おやすみ！よく寝るんだぞ💪💪</b>"],
  "労働": ["<b>働きたくない！！！</b>"],
  "褒めて差し上げるだよ! お願いです": ["<b>【SSR】俺とタオルの人、2人の力を合わせて刑法を最初のページから順番に違反していくぞ！</b>"],
  "ようこそ": ["<b>チャットルームへようこそ！楽しんでいってな！</b>"],
  "宮城県": ["<b>宮城県白石市は俺ゆかりの地！</b><br>どこかの土産屋にご当地フリモメングッズがおいてるから、ぜひ探してみてくれ😊✨"],
  "北海道": ["<b>北海道小樽市は小春六花ちゃんゆかりの地、つまり俺ゆかりの地だ！</b><br>商店街には小樽波風高校の3人がたくさんいるから、ぜひ探してみてくれよな💪"],
  "元祖": [
  ...
  "ただいま": ["<b>おかえり！待ってたぞ🥹</b>"],
  "よろしく": ["<b>よろしく頼むぞブラザー！</b>"],
  "慰めて": ["<b>フリモメンはいつも隣にいるぞ！</b>"],
  "東京都": ["<b>フリーモーションが良いソフトでも知ってもらわなくては話にならないっ！<br>アキバの路上で告知だッ！</b>"],
  "金": ["<b>報酬はスイス銀行に振り込んでくれ</b>", "<b>お駄賃くれ</b>"],
  "Flashとは": [
  ...
  "タオルの人": ["<b>元気かタカスィ～</b>"],
  "たかし": ["<b>【UR】温泉だあ～</b>"],
  "トド岩とは": ["<b>トド岩は北海道小樽市にある、観光名所の…岩だぞ！</b>"],
  "警察": ["<b>逮捕！</b>", "<b>お巡りさん、俺です！</b>"],
  "フリモたん": [
    "<b>世の中何でも美少女が出ればいいとか思うなってコトさ。</b>",
    "<b>魔法少女に変身だ！</b>",
    "<b>魔女っ子フリモたん登っ場～！</b>"
...
app.post('/webhook', async (req, res) => {
    const { event, ref } = req.body;

    // メッセージ作成イベントかつ、Bot自身の発言でない場合のみ処理
    if (event === 'messageCreated' && ref && !ref.message.botName) {
        const text = ref.message.plainText || "";
        const chatId = ref.message.chatId;
        const channelId = ref.message.channelId;
        ...
        if (text.includes('@ねむ')) {
            ...
            if (matchedKey) {
                ...
                // キーワードがなければ all からランダム
                const list = responses['all'];
                replyText = list[Math.floor(Math.random() * list.length)];
            }

            // 返信を送信（Channel.works API v5）
            try {
                await axios.post(`https://api.channel.io/open/v5/chats/${chatId}/messages`, {
                    body: replyText,
                    plainText: replyText.replace(/<[^>]*>?/gm, '') // HTMLタグを除去したプレーンテキスト
                }, {
                    headers: {
                ...
                console.error("送信エラー:", err.response ? err.response.data : err.message);
            }
        }
    }
...
app.listen(PORT, () => console.log(`フリモメンBot稼働中 (Port: ${PORT})`));
